...
// typical swap routine for lines in the line buffer
char **revLn (char **lns){
  char *temp;

  // Improv way of getting the length of a 2d string array
  int len = 0;
  while (lns[len] != 0) len++;
  
  // Pointers for the left and right side of the arrray
  char **left = lns;
  char **right = lns + len - 1;
  
  // Switching the input accordingly using a temp value for storage
  while (left < right){
    temp = left[0];
    left++[0] = right[0];
    right--[0] = temp;
...
int main (int argc, char **argv)
{
  ...
  // If we need to, reverse the order of the lines
  if (f & 2) curr = revLn(curr);
